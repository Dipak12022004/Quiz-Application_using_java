# Quiz-Application_using_java
A Java Swing-based Quiz Application with login, rules display, timed MCQs, and automated scoring.  Built using Java OOP and Swing components. Perfect for learning GUI development.

**📌 Project Overview**

 Quiz Application is a Java-based desktop application designed to make learning and testing knowledge more interactive and engaging.

The application allows users to enter their name, read quiz rules, answer multiple-choice questions within a time limit, and view their final score. It demonstrates the use of Java Swing, event handling, GUI design, and object-oriented programming concepts.

**✨ Features**

🔐 Simple login screen with name validation.
📋 Quiz rules and instructions.
❓ 10 multiple-choice questions.
⏱️ 15 seconds time limit for each question.
🎯 Next and Submit buttons.
💡 50-50 Help lifeline.
📊 Automatic score calculation.
🏆 Pass/fail status based on the score.
🔄 Play Again option.
🚪 Exit option.
🎨 User-friendly graphical interface.

**🛠️ Technologies Used**

Technology	Purpose
Java	Core programming language
Java Swing	Graphical User Interface
Java AWT	GUI components and event handling
JFrame	Application windows
JLabel	Text and labels
JButton	Buttons and actions
JRadioButton	Multiple-choice options
ButtonGroup	Select one answer
ActionListener	Handle user actions

**🖥️ Application Modules**

1. Login Module
Allows the user to enter their name.
Validates that the name is not empty.
Provides Next and Exit buttons.
2. Rules Module

Displays the quiz instructions before starting.

Total of 10 questions.
15 seconds per question.
No secondary devices allowed according to the displayed rules.
Start Quiz and Back buttons.
3. Quiz Module
Displays Java-related multiple-choice questions.
Provides four answer options.
Includes a countdown timer.
Supports Next, Submit, and 50-50 Help.
Calculates the final score.
4. Score Module
Displays the user's name.
Shows the final score.
Displays a pass or try-again message.
Provides Play Again and Exit buttons.

**🔄 Application Workflow**

Login Screen
     ↓
Enter Name
     ↓
Rules Screen
     ↓
Start Quiz
     ↓
Answer 10 Questions
     ↓
15 Seconds Per Question
     ↓
Submit Answers
     ↓
Calculate Score
     ↓
Display Result
     ↓
Play Again / Exit


**📊 Scoring System**

Total questions: 10
Marks per correct answer: 10
Maximum score: 100
Passing score: 50

The application calculates the score automatically based on the selected answers.

